...
// 제너릭 : 클래스나 메서드에서 사용할 데이터 타입을 외부에서 지정하는 방법 // 코드의 안정성, 재사용성을 위한.
...
// class 클래스명<매개타입> {}
class Test5<SCORE> { //test5라는 클래스를 만드는 순간에 SCORE 의 타입은 지정되지 않은 상태란 것.
	String subject;
	SCORE score;
}
...
class ExtendsClass<T/*매개타입을 결정하는*/ extends MyClass> {
	T variable;
}
...
public class F_Generic {
	...
	// [제어자...] <매개타입> 반환타입 메서드명(...) { ... }
	static/*제어자 집어넣고*/ <T>/*매개타입 지정*/ void genericMethod(List<T> arg) {
		for(T item: arg) {
	...
	public static void main(String[] args) {
		...
		// 해당 클래스의 혹은 하위 클래스만 제너릭으로 사용할 수 있음
		...
		// MyClass 혹은 MyClass 하위 클래스로 되어 있는 제너릭만 전달 가능
	...
	// ? : 제너릭의 와일드 카드로 모든 타입을 받을 수 있도록 함(아직 미정이다. 모르겠다)
	...
	// ? super 클래스명: 특정 클래스 혹은 특정 클래스의 상위 클래스만 타이븡로 받을 수 있도록 제한 (본인 클래스와 본인 클래스의 상위 클래스만 올 수 있는)
